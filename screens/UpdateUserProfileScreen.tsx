...
export const UpdateUserProfileScreen = ({
  route,
  navigation,
}: RootStackScreenProps<"UpdateUserProfileScreen">) => {
  const { user } = useAuthentication();
  const queryClient = useQueryClient();
  const dbRef = ref(database, `users/${user?.uid ?? "*"}/userInfo`);
  const userInfo = useDatabaseValue<UserInfo>(["userInfo", user?.uid], dbRef, {
    subscribe: true,
  });
  ...
  return (
    <View style={styles.container}>
      <Formik
        ...
        onSubmit={({ name, surname, description, voivodeship }) =>
          pushToDatabase(name, surname, description, voivodeship)
        }
      ...
      <Image
        source={require("../assets/psy/tmp3wr48z3d.png")}
